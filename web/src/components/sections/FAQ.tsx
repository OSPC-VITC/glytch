...
const FAQItem = memo(
  ({ item, index, isOpen, isHovered, onToggle, onHover }: FAQItemProps) => {
    return (
      <article
            ...
            >
              <h3
                ...
                style={{
                  ...
                  filter: isHovered
                    ? `drop-shadow(0 0 15px ${item.colors.glow})`
                    : "none",
                }}
              >
                {item.q}
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const progress = Math.max(
        0,
        Math.min(1, 1 - (rect.top - windowHeight * 0.3) / (windowHeight * 0.4)),
      );
